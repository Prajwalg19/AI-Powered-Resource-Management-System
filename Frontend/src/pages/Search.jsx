        ...
        {didFind ? (
          <>
            {result.map((smallArray, index) => (
              <div
                key={index}
                className="relative w-full max-w-4xl mb-5 overflow-x-auto shadow-md sm:rounded-lg"
              >
                {Array.isArray(smallArray[1]) && smallArray[1].length != 0 ? (
                  <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <caption className="w-full py-2 text-lg font-bold capitalize bg-gray-100 border-b-2 border-gray-400">
                      {smallArray[0]}
                    </caption>
                    <thead className="w-full text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                      <tr className="w-full">
                        {Object.keys(smallArray[1][0]).map((key, index) => (
                          <th
                            scope="row"
                            className="px-6 py-2 text-center capitalize"
                            key={index}
                          >
                            {key}
                          </th>
                          ...
                          {Object.keys(obj).map((key, index) => (
                            ...
                            >
                              ...
                              key.toLowerCase() === "remarks" ? (
                                <TextModal text={obj[key]} eq="Description" />
                              ) : (
                                obj[key]
                              )}
                            </td>
