...
function Filter() {
  ...
  function onClick(e) {
    ...
    const res = dateData.map((bigArray, index) => {
      ...
      if (Array.isArray(bigArray[1])) {
        arrayWithDate = bigArray[1].find((item) =>
          Object.keys(item).some((key) => key.includes("date"))
        );
  ...
  return (
    <>
      <main className="relative flex justify-center item-center space-x-2">
        <div>
          <button
            onClick={(e) => {
              e.preventDefault();
        ...
        {show && (
          <div className="flex items-center w-full my-2 space-x-2">
            <input
              type="date"
            ...
            <input
              ...
              id="to"
              onChange={onChange}
              className="w-1/2 mt-2 border-gray-300 rounded-md"
            />
            <button
              onClick={onClick}
              className="flex items-center justify-center w-1/2 px-2 py-2 mt-2 text-white bg-blue-500 rounded-md"
            >
              click
            </button>
          </div>
